...
//Kolejna sekcja do wyswietlania, przeszkadza przy generowaniu duzej iloœci tablic
...
int * TablicaUjemnych = new int[N]; //Tworzenie pustej tablicy gdzie zapisane beda liczby ujemne
...
int * TablicaNieujemnych = new int[N]; //Tworzenie pustej tablicy gdzie zapisane beda liczby nieujemne
...
int main()
{
    ...
    cout<<"Test wczytywania z pliku:"<<endl<<endl;
    ...
    cout<<"Test generowania Tablic:"<<endl<<endl;
    ...
    for (int ile=1000000; ile<10000000; ile+=2000000){ //For wykonuje sie 5 razy, wartoœæ iteratora jest na raz tez dlugoœcia tablicy ktora kazemy wygenerowaæ programowi. Jest ona tak duza bo co dopiero taka dlugoœæ daje jakikolwiek czas dzialania

        Tablica = GenerowanieTablic (ile,200); //Wywolanie generowania tablic o dlugosci zgodnej                                 z iteratorem for'a
        ...
        auto ms_int = duration_cast<milliseconds>(koniec - start); //Zapisujemy czas w typie double                                w formacie milisekund
        cout<<"Test dla "<<ile<<" elementow. Czas wykonania: "<<ms_int.count()<<"ms"<<endl;
        zapisywanie2<<ms_int.count()<<endl; //zapisywanie kazdego czasu do pliku
    }
    zapisywanie.close(); //zamkniecie pliku
